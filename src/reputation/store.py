...
from ..core.constants import DEFAULT_DB_FILENAME, ReputationTier
...
class ReputationStore:
    ...
    def get_record(self, session_id: str) -> ReputationRecord | None:
        """Get a record by session ID"""
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT * FROM reputation WHERE session_id = ?", (session_id,))
            row = cursor.fetchone()
            if row:
    ...
    def get_by_tier(self, tier: ReputationTier) -> list[ReputationRecord]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM reputation WHERE tier = ? ORDER BY created_at DESC", (tier.value,)
            )
            return [ReputationRecord.from_row(row) for row in cursor.fetchall()]
    ...
    def get_latest_for_profile(self, profile_id: str) -> ReputationRecord | None:
    ...
    def get_all_records(self, limit: int = 1000) -> list[ReputationRecord]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM reputation ORDER BY created_at DESC LIMIT ?", (limit,)
            )
            return [ReputationRecord.from_row(row) for row in cursor.fetchall()]
    ...
    def get_tier_counts(self) -> dict[str, int]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT tier, COUNT(*) as count FROM reputation GROUP BY tier")
    ...
    def export_audit_log(self, session_id: str) -> dict | None:
        """
    ...
    def delete_by_profile(self, profile_id: str) -> int:
        ...
        """
        with self._get_connection() as conn:
            cursor = conn.execute("DELETE FROM reputation WHERE profile_id = ?", (profile_id,))
            conn.commit()
    ...
    def cleanup_old_records(self, days: int = 30) -> int:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("DELETE FROM reputation WHERE created_at < ?", (cutoff,))
            conn.commit()
