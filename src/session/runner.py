...
class BotSessionRunner:
    ...
    def __init__(
        ...
        logger.info(
            f"BotSessionRunner initialized (session: {self.session_id}, seed: {self.behavior_seed})"
        )
    ...
    async def run(self, task: SessionTask, **task_kwargs) -> SessionResult:
        """
        Run a session task.
        ...
        try:
            ...
            await asyncio.wait_for(
                self._execute_task(task, **task_kwargs), timeout=self.timeout_seconds
            )
