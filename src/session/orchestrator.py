...
from ..adspower import AdsPowerClient, ProfileFactory, AdsPowerController
from ..proxy import ProxySessionManager, ProxyCredentials, ProxyStatsManager
from ..browser import BrowserConnector
from ..core.config import Config, get_os_name
from ..core.constants import (
    DEFAULT_MAX_PROFILES_PER_WAVE,
...
class SessionOrchestrator:
    ...
    def __init__(
        ...
        max_profiles_per_wave: Optional[int] = None,
        max_total_profiles: Optional[int] = None,
        ...
        self.max_profiles_per_wave = max_profiles_per_wave or config.orchestrator.max_profiles_per_wave
        ...
        self._adspower_client = AdsPowerClient(
            api_url=config.adspower.api_url,
        ...
        self._proxy_manager = ProxySessionManager(
            ...
            sticky_duration=config.proxy.sticky_duration
        )
        
        self._profile_factory = ProfileFactory(
            adspower_client=self._adspower_client,
    ...
    def _record_proxy_result(self, proxy_id: str, country: str, success: bool, 
    ...
    async def run_wave(
        ...
        for i in range(count):
            ...
            tasks.append(
                self._run_single_session(task, country, semaphore, wave_number, i + 1)
            )
    ...
    async def _run_single_session(
        ...
        async with semaphore:
            ...
            try:
                ...
                page = await self._connector.connect(
                ...
                if result.success:
                    ...
                    # Record proxy stats
                    latency_ms = int(result.duration_seconds * 1000) if result.duration_seconds else None
                ...
                session_id = context.session_id if context else (profile_info.session_id if profile_info else "error")
                profile_id = context.profile_id if context else (profile_info.profile_id if profile_info else "error")
