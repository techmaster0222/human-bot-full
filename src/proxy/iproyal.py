...
class IPRoyalProxy:
    ...
    def get_rotating_proxy(
        ...
        password = self._build_password(
            country=country, state=state, city=city, session_type="rotating"
        )
    ...
    def create_session(
        ...
        config, session_id = self.get_sticky_proxy(
            country=country, state=state, city=city, duration=duration
        )
        ...
        logger.info(
            f"Created sticky session: {session_id} (country: {country or self.default_country}, duration: {duration}s)"
        )
    ...
    def get_or_create_session(self, profile_id: str, country: str = None, **kwargs) -> ProxySession:
        """
    ...
    def refresh_session(self, session_id: str) -> ProxySession | None:
        """
    ...
    def test_proxy(self, config: ProxyConfig) -> bool:
        ...
        try:
            response = requests.get(
                "https://api.ipify.org?format=json", proxies=proxies, timeout=30
            )
    ...
    def get_current_ip(self, config: ProxyConfig) -> str | None:
...
class ProxyRotator:
    ...
    def get_proxy_for_profile(
        self, profile_id: str, country: str = None, sticky: bool = True, duration: int = None
    ) -> ProxyConfig:
        """
        ...
        if sticky:
            session = self.proxy_manager.create_session(
                profile_id=profile_id, country=country, duration=duration
            )
    ...
    def refresh_proxy_for_profile(self, profile_id: str) -> ProxyConfig | None:
