...
class TabFocusSimulator:
    ...
    @classmethod
    def from_context(
        cls, page: Page, context: "SessionContext", config: FocusConfig | None = None
    ) -> "TabFocusSimulator":
        """
    ...
    async def maybe_switch_tab(self) -> bool:
        ...
        # Simulate tab switch
        idle_duration = self.timing._rng.uniform(self.config.idle_min, self.config.idle_max)
    ...
    async def maybe_blur(self) -> bool:
        ...
        self._events.append(
            FocusEvent(event_type="blur", duration=pause, timestamp=self.timing._rng.random())
        )
        self._total_idle_time += pause

        return True

    async def simulate_distraction(self, duration: float | None = None):
        """
        ...
        if duration is None:
            duration = self.timing._rng.uniform(self.config.idle_min, self.config.idle_max)
        ...
        self._events.append(
            FocusEvent(
                event_type="distraction", duration=duration, timestamp=self.timing._rng.random()
            )
        )
    ...
    async def random_idle(self, min_seconds: float = 1.0, max_seconds: float = 5.0):
        ...
        self._events.append(
            FocusEvent(event_type="idle", duration=duration, timestamp=self.timing._rng.random())
        )
