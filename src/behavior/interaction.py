...
class InteractionSequencer:
    ...
    @classmethod
    def from_context(cls, page: Page, context: "SessionContext", config: Optional[InteractionConfig] = None) -> "InteractionSequencer":
        """
    ...
    async def click(
        ...
        try:
            ...
            hover_duration = self.timing._rng.uniform(
                self.config.min_hover_duration,
                self.config.max_hover_duration
            )
