...
class ScrollBehaviorEngine:
    ...
    @classmethod
    def from_context(cls, page: Page, context: "SessionContext", config: Optional[ScrollConfig] = None) -> "ScrollBehaviorEngine":
        """
    ...
    def _generate_scroll_sequence(
        ...
        while remaining > 0:
            ...
            if (last_scroll > 0 and 
                self.timing._rng.random() < self.config.back_scroll_chance):
                ...
                actions.append(ScrollAction(
                    amount=forward_amount * multiplier,
                    pause=self.timing.get_scroll_pause()
                ))
    ...
    def _get_scroll_amount(self) -> int:
        ...
        base = self.timing._rng.uniform(
            self.config.min_scroll,
            self.config.max_scroll
        )
    ...
    async def scroll_up(self, distance: Optional[int] = None) -> int:
        """Convenience method to scroll up"""
        return await self.scroll_page("up", distance)
    
    async def scroll_down(self, distance: Optional[int] = None) -> int:
