...
class MouseMovementEngine:
    ...
    def __init__(
        self, seed: int | None = None, timing_engine: TimingDistributionEngine | None = None
    ):
        """
    ...
    @classmethod
    def from_context(
        cls, context: "SessionContext", timing_engine: TimingDistributionEngine | None = None
    ) -> "MouseMovementEngine":
        """
    ...
    def generate_path(
        ...
        should_overshoot = (
            include_overshoot
            and distance > 50  # Only overshoot for longer moves
            and self._rng.random() < MOUSE_OVERSHOOT_CHANCE
        )
        ...
        if should_overshoot:
            ...
            main_path = self._generate_bezier_path(
                start, overshoot_target, num_points=int(num_points * 0.8)
            )

            # Correction path back to target
            correction_path = self._generate_correction_path(
                overshoot_target, end, num_points=MOUSE_CORRECTION_STEPS
            )
    ...
    def _generate_bezier_path(self, start: Point, end: Point, num_points: int) -> list[PathPoint]:
    ...
    def _generate_control_points(self, start: Point, end: Point) -> list[Point]:
        ...
        cp1_offset = Point(
            self._rng.uniform(-max_offset, max_offset), self._rng.uniform(-max_offset, max_offset)
        )
        cp2_offset = Point(
            self._rng.uniform(-max_offset, max_offset), self._rng.uniform(-max_offset, max_offset)
        )
    ...
    def _calculate_overshoot(self, start: Point, end: Point) -> Point:
        ...
        rotated_dir = Point(
            norm_dir.x * cos_a - norm_dir.y * sin_a, norm_dir.x * sin_a + norm_dir.y * cos_a
        )

        return Point(
            end.x + rotated_dir.x * overshoot_distance, end.y + rotated_dir.y * overshoot_distance
        )

    def _generate_correction_path(
        self, overshoot: Point, target: Point, num_points: int
    ) -> list[PathPoint]:
    ...
    def _add_noise(self, path: list[PathPoint]) -> list[PathPoint]:
        ...
        for i, pp in enumerate(path):
            ...
            noisy_point = Point(pp.point.x + noise_x, pp.point.y + noise_y)
            noisy_path.append(PathPoint(point=noisy_point, delay=pp.delay, velocity=pp.velocity))

        return noisy_path

    def _calculate_timing(self, path: list[PathPoint], total_distance: float) -> list[PathPoint]:
        ...
        _ = total_distance / base_velocity  # Calculate for reference
    ...
    def add_overshoot(self, path: list[PathPoint], target: Point) -> list[PathPoint]:
        """
        ...
        correction = self._generate_correction_path(
            last_point, target, num_points=MOUSE_CORRECTION_STEPS
        )

        return path + correction

    def get_movement_time(self, start: Point, end: Point) -> float:
        """
