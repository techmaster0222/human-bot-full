...
class TimingDistributionEngine:
    ...
    def __init__(self, seed: int | None = None, config: TimingConfig | None = None):
        """
    ...
    @classmethod
    def from_context(
        cls, context: "SessionContext", config: TimingConfig | None = None
    ...
    def timing_stats(self) -> dict:
        ...
        return {
            ...
            "click_mean": (
                self._pareto_click.mean() * self.config.click_min
                if self.config.click_alpha > 1
                else "undefined"
            ),
        }
