...
class BotSession:
    ...
    def start(self, duration: int = None) -> bool:
        ...
        if duration:
            ...
            self.target_duration = random.randint(
                self.session_config.min_duration,
                self.session_config.max_duration
            )
    ...
    async def type_in_field(
        ...
        result = await self.actions.type_text(
            selector, text,
            clear_first=clear,
            submit=submit
        )
    ...
    def get_requests_by_type(self, resource_type: str) -> List[Dict]:
        ...
        return [
            r for r in self.actions.get_network_requests()
    ...
    async def _maybe_take_break(self):
        ...
        if random.random() < self.session_config.break_chance:
            break_duration = random.randint(
                self.session_config.break_min_duration,
                self.session_config.break_max_duration
            )
...
class SessionManager:
    ...
    def get_session(self, profile_id: str) -> Optional[BotSession]:
