...
class HumanBehavior:
    ...
    def get_typing_delays(self, text: str) -> List[Tuple[str, float]]:
        ...
        for i, char in enumerate(text):
            ...
            if self.config.typo_chance > 0 and random.random() < self.config.typo_chance:
                ...
                result.append((char, random.uniform(
                    self.config.typing_min_delay,
                    self.config.typing_max_delay
                )))
            else:
    ...
    def get_scroll_sequence(
        ...
        while scrolled < total_distance:
            ...
            # Add scroll action
            delay = random.uniform(
                self.config.scroll_pause * 0.5,
                self.config.scroll_pause * 1.5
            )
    ...
    def get_click_delay(self) -> float:
        ...
        if self.config.hover_before_click:
            return random.uniform(
                self.config.hover_duration * 0.7,
                self.config.hover_duration * 1.3
            )
        return 0.01
