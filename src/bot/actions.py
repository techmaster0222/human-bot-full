...
class BotActions:
    ...
    async def navigate_to(
        self, url: str, wait_until: str = "domcontentloaded", timeout: int = None
    ) -> Response | None:
        """
    ...
    async def click(
        self, selector: str, natural_movement: bool = True, timeout: int = None, force: bool = False
    ) -> bool:
        """
    ...
    async def _natural_mouse_move(self, target_x: float, target_y: float):
        ...
        try:
            ...
            path = self.behavior.generate_mouse_path(
                (int(current_x), int(current_y)), (int(target_x), int(target_y))
            )
    ...
    async def wait_for_network_idle(self, timeout: int = None):
        ...
        await self.page.wait_for_load_state("networkidle", timeout=timeout or self.default_timeout)
