...
class ProfileState:
    ...
    cdp_endpoint: Optional[CDPEndpoint] = None
    start_count: int = 0
...
class AdsPowerController:
    ...
    async def stop_profile(self, profile_id: str) -> bool:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: self.client.stop_browser(profile_id)
        )
    ...
    async def delete_profile(self, profile_id: str) -> bool:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: self.client.delete_profile([profile_id])
        )
    ...
    def get_cdp_endpoint(self, profile_id: str) -> Optional[str]:
        """
    ...
    async def check_status(self, profile_id: str) -> bool:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: self.client.check_browser_status(profile_id)
        )
