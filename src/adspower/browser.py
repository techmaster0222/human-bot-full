...
class BrowserController:
    ...
    async def _connect_playwright(self, cdp_endpoint: str, profile_id: str) -> Optional[Page]:
        """
        ...
        try:
            ...
            logger.debug(f"Playwright connected to CDP endpoint")
            return page
    ...
    def get_context(self, profile_id: str) -> Optional[BrowserContext]:
...
class SyncBrowserController:
    ...
    def start_browser(self, profile: Profile, **kwargs) -> Optional[Page]:
    ...
    def stop_browser(self, profile_id: str) -> bool:
        return self._get_loop().run_until_complete(
            self._async_controller.stop_browser(profile_id)
        )
    
    def stop_all_browsers(self) -> int:
        return self._get_loop().run_until_complete(
            self._async_controller.stop_all_browsers()
        )
